	...
	//
{
	...
	SimProjGui( int W, int H, const char *l = 0 )
		: DoubleWindow( W, H, l ),
	...
	void PropertyChanged( void* sender, const string& propertyName ) override {
		// cerr << "SimProjGui: Received PropertyChanged(," << propertyName << ")\n";
	...
	void WireUp() {
		...
		partition.AddConsumer( &rank );

		rank.AddPropertyChangedEventHandler( this );
		rank.getDbSeqs = getDatabase;
		rank.getDbIndex = getSeqIndex;
		rank.getTestSetIndex = getTestSetIndex;
		rank.getTrainingSetIndex = getTrainingSetIndex;
		rank.getMatrix = getMatrix;
...
int main( int argc, char *argv[] ) {
	...
	dlg = new SimProjGui( 1024, 768 );
	dlg->SetSettingName( argc > 1 ? argv[1] : "SimProj.cfg" );
	dlg->LoadSettings();
