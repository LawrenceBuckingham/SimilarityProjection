...
#pragma endregion
	...
	static void RunImpl_( RankSimProj * self ) {
		self->RunImpl();
	}
	 ...
	 */
	static void Setup(
		const decltype(RankSimProj::dbSeqs) database,
		const decltype(RankSimProj::testSetIndex) queryIndices,
		int maxMatches,
		vector<::Ranking> & rankings
	) {
		rankings.clear();
	...
	static void RankQuery(
		...
		const decltype(RankSimProj::dbSeqs) dbSeqs,
		const decltype(RankSimProj::trainingSetIndex) trainingSetIndex,
		uint kmerLength,
...
#endif
			for ( uint i = 0; i < trainingSetIndex->size(); i++ ) {
				auto subject = (*dbSeqs)[(*trainingSetIndex)[i]];
#if WANT_FRAG
				...
				vector<Distance> rowMin( queryFragCount, numeric_limits<Distance>::max() );
				vector<Distance> colMin( subjectFragCount, numeric_limits<Distance>::max() );
#else
				...
				if ( knn.canPush( distance ) ) {
					knn.push( subject, distance );
				}
			}
...
#pragma omp critical
			...
			{
				for ( auto e : knn ) {
					if ( ranking.knn.canPush( e.first ) ) {
						ranking.knn.push( e.second, e.first );
					}
				}
			}
		}
...
#pragma omp critical
	...
	void RankMinMin( ProgressFunc && progress ) {
		int done = 0;
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMin, CombineMin, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMin, CombineMin, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}

	void RankMinMax( ProgressFunc  && progress ) {
		int done = 0;
		bool useDigrams = UseDigrams();
#if WANT_FRAG
		auto fragLength = FragLength();
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMin, CombineMax, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMin, CombineMax, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}

	void RankMinMean( ProgressFunc  && progress ) {
		int done = 0;
		bool useDigrams = UseDigrams();
#if WANT_FRAG
		auto fragLength = FragLength();
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMin, CombineMean, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMin, CombineMean, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}

	void RankMaxMin( ProgressFunc  && progress ) {
		int done = 0;
		bool useDigrams = UseDigrams();
#if WANT_FRAG
		auto fragLength = FragLength();
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMax, CombineMin, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMax, CombineMin, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}

	void RankMaxMax( ProgressFunc  && progress ) {
		int done = 0;
		bool useDigrams = UseDigrams();
#if WANT_FRAG
		auto fragLength = FragLength();
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMax, CombineMax, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMax, CombineMax, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}

	void RankMaxMean( ProgressFunc  && progress ) {
		int done = 0;
		bool useDigrams = UseDigrams();
#if WANT_FRAG
		auto fragLength = FragLength();
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMax, CombineMean, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMax, CombineMean, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}

	void RankMeanMin( ProgressFunc  && progress ) {
		int done = 0;
		bool useDigrams = UseDigrams();
#if WANT_FRAG
		auto fragLength = FragLength();
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMean, CombineMin, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMean, CombineMin, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}

	void RankMeanMax( ProgressFunc  && progress ) {
		int done = 0;
		bool useDigrams = UseDigrams();
#if WANT_FRAG
		auto fragLength = FragLength();
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMean, CombineMax, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMean, CombineMax, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}

	void RankMeanMean( ProgressFunc  && progress ) {
		int done = 0;
		bool useDigrams = UseDigrams();
#if WANT_FRAG
		auto fragLength = FragLength();
		...
		for ( size_t q = 0; q < testSetIndex->size(); q++ ) {
			auto query = (*dbSeqs)[(*testSetIndex)[q]];
#if WANT_FRAG
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, fragLength, ReduceMean, CombineMean, useDigrams, saveMatrixFile, rankings[q] );
#else
			RankQuery( query, dbSeqs, trainingSetIndex, kmerLength.Value(), matrix, ReduceMean, CombineMean, useDigrams, saveMatrixFile, rankings[q] );
#endif
			progress( done );
		}
	}
	...
	void Rank() {
		...
		Setup( dbSeqs, testSetIndex, maxMatches, rankings );
		...
		if ( ReduceMode() == "min" && CombineMode() == "min" ) RankMinMin( progress );
		else if ( ReduceMode() == "min" && CombineMode() == "max" ) RankMinMax( progress );
		else if ( ReduceMode() == "min" && CombineMode() == "mean" ) RankMinMean( progress );
		else if ( ReduceMode() == "max" && CombineMode() == "min" ) RankMaxMin( progress );
		else if ( ReduceMode() == "max" && CombineMode() == "max" ) RankMaxMax( progress );
		else if ( ReduceMode() == "max" && CombineMode() == "mean" ) RankMaxMean( progress );
		else if ( ReduceMode() == "mean" && CombineMode() == "min" ) RankMeanMin( progress );
		else if ( ReduceMode() == "mean" && CombineMode() == "max" ) RankMeanMax( progress );
		else if ( ReduceMode() == "mean" && CombineMode() == "mean" ) RankMeanMean( progress );
	...
	static void LoadImpl_( RankSimProj * self ) {
		self->LoadImpl();
	}
