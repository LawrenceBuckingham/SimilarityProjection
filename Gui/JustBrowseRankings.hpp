...
#pragma region PrecisionRecallFileName Property
private:
	TypedArg<string, OutFileChooser> precisionRecallFileName;

public:
	string PrecisionRecallFileName() {
		return precisionRecallFileName.Value();
	}

	void SetPrecisionRecallFileName( const string val ) {
		if ( val == PrecisionRecallFileName() ) return;

		this->precisionRecallFileName.SetValue( val );
	}
#pragma endregion
...
public:
	JustBrowseRankings(
		...
		Page( left, top, width, height, "Just Browse Rankings" ),
		...
		rankingFileName(
			...
			"The path to a file from which rankings will be loaded, in CSV \n"
			"format.",
		...
		precisionRecallFileName(
			"precisionRecallFileName", "", "Precision Recall File", Requirement::Required,
			"The path to a file which will be overwritten with detailed \n"
			"interpolated precision-recall data, in CSV format.",
			labelWidth, 0, width, rowHeight ),
		...
		//
	{
		...
		stack.add( &autoLoad );
		stack.add( &precisionRecallFileName );

		rankingFileName.AddPropertyChangedEventHandler( this );
		rankingFileName.inputField.SetPattern( "*.rankings.csv" );

		precisionRecallFileName.AddPropertyChangedEventHandler( this );
		precisionRecallFileName.inputField.SetPattern( "*.precision_recall.csv" );
...
#pragma endregion
	...
	static void RunImpl_( JustBrowseRankings * self ) {
		self->RunImpl();
	}
	...
	void RunImpl() {
		...
		// Load existing data, then save it again, with precision and recall added when missing.
		Load();
		// but that has been removed from this stub.
		Page::runTime += omp_get_wtime();
	...
	static void LoadImpl_( JustBrowseRankings* self ) {
		self->LoadImpl();
	}
	...
	void SaveStats() {
		auto fileName = String::Trim( PrecisionRecallFileName() );

		if ( fileName.length() == 0 ) return;

		ofstream precisionRecallFile(fileName);

		CsvWriter out(precisionRecallFile);

		for ( size_t i = 0; i < rankings.size(); i++ ) {
			auto & ranking = rankings[i];
			out << ranking.sequence->IdStr();
			auto & record = stats.prec[i];
			for ( auto p: record ) out << p;
			out.Ln();
		}
	}
	...
	void Load() {
		...
		auto setMaxMatches = [this]( size_t maxMatches ) {
			UPDATE_GUI( disp( "%zu rankings loaded.\n", rankings.size() ); );
		};
		...
		try {
			...
			ShowProgress( "Error parsing rankings from file %s.\nSource Location: %s(%d)\nReason:%s\n", fileName.c_str(), ex.File(), ex.Line(), ex.what() );
		}
	...
	void UpdateMissing() {
		...
		for ( auto & ranking : rankings ) {
			seen.insert( ranking.sequence->Id() );
		}

		auto capacity = Util::Max( rankings.begin(), rankings.end(), []( const ::Ranking & r ) { return r.knn.capacity; }, 0ull );
		vector<double> empty;

		for ( auto idx : (*testSetIndex) ) {
			auto seq = (*dbSeqs)[idx];

			if ( seen.count( seq->Id() ) == 0 ) {
				KnnVector<const FastaSequence *, double> knn( capacity, -HUGE_VAL );
				rankings.emplace_back( seq, knn, empty, empty );
			}
		}
	}
...
#define addVar(x,fmt) parms.emplace( Name(), STRING(x), String::Format(fmt,x) )
		...
		addPar( precisionRecallFileName );
...
#define setVar(x,t) if ( p.ParamName() == STRING(x) ) (x) = p.Value<t>()
		for ( auto & p : parms ) {
			...
			setPar( precisionRecallFileName, string );
